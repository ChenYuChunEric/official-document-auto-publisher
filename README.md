...
註1：點擊右側Release取得.exe應用程式檔案

註2：下載的壓縮公文檔案及解壓縮轉換完的檔案可以指定放在任意想要的資料夾內(開啟程式後可以選擇去哪裡尋找及要放在哪裡)

## 操作流程
- **從公文系統下載好要轉換和發佈的公文(保持壓縮狀態)，檔案位址可以任意自訂(後續程式執行中可指定來源和轉換後的資料夾)
- **至右側Releasr取得.exe應用程式檔案
- **下載的.exe檔案可以放在任意位置，第一次開啟後會自動生成一個config.json文字格式檔案(按右鍵以記事本開啟編輯)
...
- **存檔後關閉.exe應用程式及config.json，並再次開啟.exe應用程式
- **依照彈跳視窗流程完成即可自動開啟瀏覽器並自行填入單一身份認證帳號密碼(程式不會儲存也不會知道)
- **按下"確認登入"後由電腦接手完成剩餘發佈步驟(可將視窗縮到最小)
